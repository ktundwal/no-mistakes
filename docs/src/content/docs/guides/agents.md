...
When the trusted repository config sets [`disable_project_settings: true`](/no-mistakes/reference/repo-config/#disable_project_settings), Copilot receives `--no-custom-instructions` and `--allow-all-paths`, then runs from an empty per-invocation working directory with the absolute target worktree bound in its prompt. Its private per-invocation `COPILOT_HOME` also enables the supported `disableAllHooks` setting. Keeping Copilot's discovery root outside the target blocks repository `AGENTS.md`-style instructions, settings, and `.github/hooks` even when a trusted folder tries to re-enable hooks; the private config root carries only authentication state and BYOK provider definitions needed to keep Copilot usable. The gate fails closed if an override selects a custom agent, another config or working root, a resumable session, or a conflicting value-bearing suppression flag. This behavior is verified with Copilot CLI 1.0.86-2.
Any other `agent_args_override.copilot` flags are inserted before no-mistakes' ordinary managed flags, so user choices take effect. Prefer [`agent_config.copilot`](/no-mistakes/reference/global-config/#agent_config) for model and reasoning effort; it renders the same `--model` and `--effort` flags, and a raw flag here still wins over it. When `disable_project_settings` is false, no-mistakes leaves the operator's normal `COPILOT_HOME` and project hook behavior unchanged.
